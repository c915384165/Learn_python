...
def auto_plotS(input_csv_file_name, y_column_number, plt_x_label, plt_y_label, save_fig_name,
              fig_title, ylim_min, ylim_max, x_min, x_max):
    file_name = input_csv_file_name
    df = pd.read_csv(file_name)
    # CSV in date&time
    df['date&time'] = pd.to_datetime(df['date&time'])
...
#    plt.xlim(my_time_date[0], my_time_date[-1])
    ...
    plt.grid(True)
#    plt.xticks(pd.date_range('2019-06-01','2019-07-22'),rotation=90)
#    plt.xlim(right=3)
#    plt.ylim()
    plt.scatter(my_time_date, a)
#    plt.gcf().autofmt_xdate()
    plt.savefig(save_fig_name, dpi=128)
...
#    plt.xlim(my_time_date[0], my_time_date[-1])
    ...
    plt.ylim(ylim_min,ylim_max)
    plt.grid(True)
    plt.xlim(my_time_date[x_min], my_time_date[x_max])
#    plt.ylim()
    plt.plot(my_time_date, a)
    plt.gcf().autofmt_xdate()
    plt.savefig(save_fig_name, dpi=128)
...
def auto_plot_numD(input_csv_file_name, y_column_number, fig_title, ylim_min, ylim_max, x_min, x_max):
    index_name_list = []
    with open(input_csv_file_name) as f:
        reader = csv.reader(f)
        header_row = next(reader)
    ...
    for i in y_column_number:
        ...
        auto_plotD(input_csv_file_name, index_name_list[i], '日期',
                 index_name_list[i], fig_name, fig_title, ylim_min, ylim_max,x_min, x_max)
        auto_analysis(input_csv_file_name, index_name_list[i], index_name_list[i])


def auto_plot_numS(input_csv_file_name, y_column_number, fig_title, ylim_min, ylim_max, x_min, x_max):
    index_name_list = []
    with open(input_csv_file_name) as f:
        reader = csv.reader(f)
        header_row = next(reader)
    ...
    for i in y_column_number:
        fig_name = fig_title + '_fig-' + str(i)
        auto_plotS(input_csv_file_name, index_name_list[i], '日期',
                 index_name_list[i], fig_name, fig_title, ylim_min, ylim_max, x_min, x_max)
        auto_analysis(input_csv_file_name, index_name_list[i], index_name_list[i])
